module.exports = {
    ...
    unTrackBand: async (req, res) => {
        const {band_name} = req.body;
        const db = req.app.get('db');

        const trackedArtist = db.post.checkIfTracking(req.session.user.user_id, band_name).then(count => {
            console.log(count[0]);
            if(count[0].count == '1') {
            db.post.unTrackBand(req.session.user.user_id, band_name);
            res.status(200).json(trackedArtist)
        } else {
